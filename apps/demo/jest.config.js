...
module.exports = {
  ...
  displayName: 'demo',
  rootDir: '../..',
  testMatch: ['<rootDir>/apps/demo/src/**/*.spec.ts'],
  setupFilesAfterEnv: ['<rootDir>/apps/demo/src/test-setup.ts'],
  transform: {
    ...ngPreset.transform,
    '^.+\\.(ts|js|mjs|html|svg)$': [
      'jest-preset-angular',
      { tsconfig: '<rootDir>/apps/demo/tsconfig.spec.json', stringifyContentPathRegex: '\\.(html|svg)$' },
    ],
  },
  moduleNameMapper: {
