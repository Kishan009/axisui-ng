...
module.exports = {
  ...
  rootDir: '../..',
  testMatch: ['<rootDir>/libs/overlays/src/**/*.spec.ts'],
  setupFilesAfterEnv: ['<rootDir>/libs/overlays/src/test-setup.ts'],
  transform: {
    ...ngPreset.transform,
    '^.+\\.(ts|js|mjs|html|svg)$': [
      'jest-preset-angular',
      { tsconfig: '<rootDir>/libs/overlays/tsconfig.spec.json', stringifyContentPathRegex: '\\.(html|svg)$' },
    ],
  },
  moduleNameMapper: {
  ...
  coverageDirectory: '<rootDir>/coverage/libs/overlays',
};
