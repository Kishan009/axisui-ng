...
module.exports = {
  ...
  rootDir: '../..',
  testMatch: ['<rootDir>/libs/feedback/src/**/*.spec.ts'],
  setupFilesAfterEnv: ['<rootDir>/libs/feedback/src/test-setup.ts'],
  transform: {
    ...ngPreset.transform,
    '^.+\\.(ts|js|mjs|html|svg)$': [
      'jest-preset-angular',
      { tsconfig: '<rootDir>/libs/feedback/tsconfig.spec.json', stringifyContentPathRegex: '\\.(html|svg)$' },
    ],
  },
  moduleNameMapper: {
  ...
  coverageDirectory: '<rootDir>/coverage/libs/feedback',
};
